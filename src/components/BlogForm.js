...
const BlogForm = ({edit}) => {
    ...
    const onSubmit = ()=>{
        ...
        if(validate()===true){
            if(userImg){
                ...
                uploadBytes(imageRef, userImg).then((res)=>{
                    getDownloadURL(res.ref).then((res)=>{
                        ...
                        if(edit){
                            axios.put(`http://localhost:3002/${isAdmin? 'adminPosts':'posts'}/${id}`,{
                                title,
                                body,
                                category,
                                date: formattedDate,
                        ...
                        const comments = [];
                        //데이터 post
                        axios.post(`http://localhost:3002/${isBoardAdmin? 'adminPosts':'posts'}`,{
                            title,
                            body,
                            category,
                            date: formattedDate,
            ...
            const comments = [];
            //데이터 post
            axios.post(`http://localhost:3002/${isAdmin? 'adminPosts':'posts'}`,{
                title,
                body,
                category,
                date: formattedDate,
